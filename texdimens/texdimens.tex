...
%
% But this means (see the nota bene above) that we could apply this procedure
% also for f>=65536, because at worst we will get a \the\numexprk+1.0, which
% gives the correct result. This would spare us the branching and now we
% are in the usual conundrum: should we test at all or not if dim2<1pt?
% If the \ifnum costs more than the overhead it tries then to spare
% for the dim2>=1pt case, then we should drop this \ifnum branching!
