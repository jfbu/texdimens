...
%
% dim2>0pt was initially assumed.
% To handle dim2<0pt, we simply simultaneously do
% dim1<-- -dim1 and dim2<-- -dim2. The case dim2=0pt is not intercepted
% and will cause division by zero low-level error.
...
% arithmetic overflow.
%
% Negative #2 are converted into positive #2 by replacing both
% arguments by their opposites.
% The case #2=1pt=65536sp is special, and must be filtered out (issues #4, #6).
...
\def\texdimenwithunit_i#1{\texdimengobtominus#1\texdimenwithunit_switchsigns-%
                          \texdimenwithunit_j#1}%
...
% with -, as -0 is bad there. So let's check also if #2 is 0
    \texdimenzerominusfork
      #2-\texdimenwithunit_Bzero % abuse
      0#2\texdimenwithunit_j     % also
       0-{\texdimenwithunit_ic#2}% I should have left the #2 after \krof
    \krof
    #1;%
}%
...
\def\texdimenwithunit_A#1#2;#3;{%
    ...
    % with *\p@ better but an early doubled dim2 would complicate 1pt test
}%
...
\def\texdimenwithunit_Bb#1;#2;{%
    % if I could replace #2 by 2*#2 with ease, _Bc could use \p@
