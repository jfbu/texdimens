...
% as the smalleast attainable one larger than Tsp.
%
...
% We know how to get largest "X sp <= T sp" exactly
% expressible in "mm", i.e. means X=trunc(a 7227/2540) for some integer a.
% We want to achieve X=trunc(b 803/800)=trunc(b 7227/7200) for some b.
% Only the congruence of X modulo 803 matters and it turns out that
% the excluded values for X mod. 803 are 267, 535, 802.
% (see issue #10 for the corresponding 9 excluded values of a modulo 2540
% found by Ruixi Zhang; a<-a+2540 makes X<-X+9*803 hence X mod. 803
% is invariant; turns out this excludes in total 3+3+3=9 "a" values).
%
% If X is not excluded we are done, if it is excluded, we replace a<-a-1.
%
% Implementation update: the check if X sp is attainable via bp unit is
% *not* done via computation of X mod 803 but via the math of
% \texdimenbp to check if its output is same (in sp unit) as X.
%
% Automatically
% the new X (which differs from initial one by -2 or -3) will be allowed
% and we are now done. To express the final output in "mm" unit, we use
% the found adjusted "a". To express it in "sp" or "bp" unit, we do
% compute the new X then we are one in "sp", and we get it in "bp".
%
% In this first round of I will not try to share macros among bothbpmm,
% bothmmbp, or bothbpmmsp/bothbpmmpt. (I provided bothincmpt, but would
% it not have been better only bothincmsp? I will only do bothbpmmsp)
...
\def\texdimenbothbpmm_a#1;{\expandafter\texdimenbothbpmm_b
    \the\numexpr#1*2540/7227;#1;}%
% we get the X from this candidate, hijacking TeX built-in *7227/2540
...
\def\texdimenbothbpmm_c#1;#2;#3;{\ifnum#1>#3 \expandafter\texdimenbothbpmm_ca\fi
    \texdimenbothbpmm_d#1;#2;}%
% Here we
% check if X sp is expressible in bp unit. If not we decrease "a" by 1.
% The check could be done by computing X modulo 803
% but we do otherwise, using the math underlying the \texdimenbp macro.
% Recall that all this expansion is after an
% \expandafter\texdimenstrippt\the\dimexpr, and the decimal multiple of
% pt obtained from \the\dimexpr "a"sp=D pt is such that Dmm matches
% trunc(a 7227/2540)sp. We don't need for the \texdimenbothbpmm
% the actual value of the latter if a needed adjustment.
\def\texdimenbothbpmm_d#1;#2;{#2sp%
    \ifnum\dimexpr
    \expandafter\texdimenstrippt\the\dimexpr\numexpr(2*#1+1)*400/803spbp=#1
    \else-1sp\fi\relax}%
% Here we must decrease "a=#2" by 1 and recompute X=#1, then we loop
% back to the final possible adjustment of "a" done in \texdimenbothbpmm_d
\def\texdimenbothbpmm_ca\texdimenbothbpmm_d#1;#2;{%
    \expandafter\texdimenbothbpmm_cb\the\numexpr#2-1;%
}%
\def\texdimenbothbpmm_cb#1;{\expandafter\texdimenbothbpmm_d
    \the\numexpr\dimexpr\expandafter\texdimenstrippt\the\dimexpr#1spmm;#1;}%
% done...
% now the lazy way for \texdimenbothmmbp
\def\texdimenbothmmbp#1{\expandafter\texdimenstrippt\the\dimexpr
                        \expandafter\texdimenbothmmbp_fork\the\numexpr\dimexpr\texdimenbothbpmm{#1}mm;}%
\def\texdimenbothmmbp_fork#1{\texdimenzerominusfork
                             #1-\texdimenbothmmbp_zero
                             0#1\texdimenbothmmbp_neg
                             0-\texdimenbothmmbp_a
                             \krof#1}%
\def\texdimenbothmmbp_zero#1;{\z@\relax}%
\def\texdimenbothmmbp_neg-{-\texdimenbothmmbp_a{}}%
\def\texdimenbothmmbp_a#1;{\numexpr(2*#1+1)*400/803sp\relax}%
% and \texdimenbothbpmmpt
\def\texdimenbothbpmmpt#1{\texdimenpt{\texdimenbothbpmm{#1}mm}}%
% and \texdimenbothbpmmsp
