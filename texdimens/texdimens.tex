...
% f=65536: merged with f<65536 branch (as it turns out the latter
%          works with it, and 1pt as second argument need not be handled
%          preferentially)
% f<65536: 0.99 release used the round((2T+1)*32768/f) formula
%          (it is funny it works for all f except f=65536)
%
...
%
%          1.0 release thus uses the ceil(T*65536/f) formula, as it is the
...
%          overflow.  Let's try this to reduce to a numexpr rounding <x/y>:
%
...
%          with trunc(C*f/65536) exactly identical with r.
%
%          So trunc(R*f/65536) will be either r, then R=C, or r-1,
%          then R=C-1.
%
%          Method from release 1.0: 
% 
%          Let's TeX compute P=trunc(R*f/65536) itself!
%          We do this via P sp = E <f sp> where E is
...
%          New method: 
%
%          Get the sign of R-x, i.e. the sign of
%          R*f-65536*r.  How to avoid overflow in this?
%
...
%
%              Return the contatenation <digits of k>.d...d
%
...
% In a later commit we will leave the k=#1 upfront and avoid having to
% grab it again, but we will benchmark that later. So provisory here.
\def\texdimenwithunit_Bb#1;#2;#3;{%
    \expandafter\texdimenwithunit_Bc\the\numexpr#2-#1*#3;#1;#3;%
}%
% now r;k;f;. Get R=<r*65536/f>
...
\def\texdimenwithunit_Bd#1;#2;#3;{%
    \expandafter\texdimenstrippt
    \the\dimexpr\numexpr#1%
% Never tested if \ifnumA<B better with checking sign of A-B.
% (indeed, very little usage of \ifnum in xint, so no need to test in
% the past). We will use this for time being.
