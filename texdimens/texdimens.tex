...
\def\texdimenbothincm#1{\expandafter\texdimenic_a\the\numexpr\dimexpr#1;127);}%
\def\texdimenbothcmin#1{\expandafter\texdimenic_a\the\numexpr\dimexpr#1;50);}%
\def\texdimenbothincmpt#1{\expandafter\texdimenic_a\the\numexpr\dimexpr#1;7227+1)/2-1;}%
...
\def\texdimenbothincmsp#1{\the\numexpr\expandafter\texdimenicsp_a\the\numexpr\dimexpr#1;}%
\def\texdimenicsp_a#1{\if-#1\texdimenicsp_neg\fi\texdimenicsp_b#1}%
\def\texdimenicsp_b#1;{(((2*#1-3612)/7227)*7227+1)/2-1\relax}%
% #1 is \fi
% we need to regrab or we add a \numexpr..\relax layer to \texdimenicsp_b
% (parentheses won't do as \numexpr chokes on -(...))
\def\texdimenicsp_neg#1#2-#3;{#1-\numexpr#2#3;\relax}%
%
